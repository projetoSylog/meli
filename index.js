...
app.get('/home', validateToken, async (req, res) => {
  try {
    //var meliObject = new MeliObject(res.locals.access_token);
...
app.get('/post', validateToken, upload.single('picture'), async (req, res) => {
  try {
...
app.get('/notifications', (req, res) => {
  res.send('ok');
  const corpoRequisicaoResult = req.body;
  ...
  fetch("https://mercadolivre.sylog.com.br/api/notificaml/notifica",
{
    headers: {
    ...
    method: "POST",
    body: JSON.stringify(corpoRequisicaoResult)
})
